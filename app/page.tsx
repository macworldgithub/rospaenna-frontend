          ...
          <div className="border border-gray-200 bg-white">
            ...
            <div className="p-5 flex flex-col gap-6">
              ...
              <div>
                ...
                <div ref={sliderRef} className="relative h-1 bg-gray-300 rounded-full mt-3 select-none">
                  <div 
                    className="absolute h-full bg-[#3a4550] rounded-full"
                    style={{ left: `${((minTime - 5) / 15) * 100}%`, right: `${100 - ((maxTime - 5) / 15) * 100}%` }}
                  ></div>
                  <div 
              ...
              <div>
                ...
                <div className="flex flex-col gap-3">
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="course" value="old-tom-morris" checked={selectedCourse === 'old-tom-morris'} onChange={(e) => setSelectedCourse(e.target.value)} className="hidden" />
                    <div className={`w-4 h-4 rounded-full border flex items-center justify-center p-[2px] ${selectedCourse === 'old-tom-morris' ? 'border-[#1a8bc4]' : 'border-gray-300 group-hover:border-gray-400'}`}>
                      {selectedCourse === 'old-tom-morris' && <div className="w-full h-full bg-[#1a8bc4] rounded-full"></div>}
                    </div>
                    <span className={`text-[13px] ${selectedCourse === 'old-tom-morris' ? 'text-gray-700 group-hover:text-black' : 'text-gray-500 group-hover:text-gray-700'}`}>Old Tom Morris Links</span>
                  </label>
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="course" value="sandy-hills" checked={selectedCourse === 'sandy-hills'} onChange={(e) => setSelectedCourse(e.target.value)} className="hidden" />
                    <div className={`w-4 h-4 rounded-full border flex items-center justify-center p-[2px] ${selectedCourse === 'sandy-hills' ? 'border-[#1a8bc4]' : 'border-gray-300 group-hover:border-gray-400'}`}>
                      {selectedCourse === 'sandy-hills' && <div className="w-full h-full bg-[#1a8bc4] rounded-full"></div>}
                    </div>
                    <span className={`text-[13px] ${selectedCourse === 'sandy-hills' ? 'text-gray-700 group-hover:text-black' : 'text-gray-500 group-hover:text-gray-700'}`}>Sandy Hills Links</span>
                  </label>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex bg-[#f5f5f5] rounded-[3px] border border-gray-200 overflow-hidden text-xs font-bold shadow-sm">
                  <button onClick={() => setGroupBy('price')} className={`flex-1 py-2.5 ${groupBy === 'price' ? 'bg-[#3a4550] text-white' : 'text-gray-400 hover:bg-gray-50 transition-colors'}`}>Price</button>
                  <button onClick={() => setGroupBy('time')} className={`flex-1 py-2.5 ${groupBy === 'time' ? 'bg-[#3a4550] text-white' : 'text-gray-400 hover:bg-gray-50 transition-colors'}`}>Time</button>
                </div>
              </div>
            ...
            <>
              {isRefreshing ? (
                ...
                <div className="bg-[#56606a] text-white px-5 py-3 flex justify-between items-center font-bold tracking-wide uppercase text-[12px] rounded-t-[3px]">
                  <span>1. Standard Visitor - {selectedCourse === 'old-tom-morris' ? 'Old Tom Morris' : 'Sandy Hills Links'}</span>
              ...
              <div className="shadow-sm">
                <div className="bg-[#56606a] text-white px-5 py-3 flex justify-between items-center font-bold tracking-wide uppercase text-[12px] rounded-t-[3px]">
                  <span>2. Irish Resident - {selectedCourse === 'old-tom-morris' ? 'Old Tom Morris' : 'Sandy Hills Links'}</span>
